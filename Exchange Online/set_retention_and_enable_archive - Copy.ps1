...
try {
    ...
    Get-EXOMailbox -ResultSize Unlimited -PropertySets Minimum | ForEach-Object {
        ...
        $isSharedMailbox = ($_.RecipientTypeDetails -eq 'SharedMailbox')
        ...
        try {
            ...
            try {
                $filter = [System.Uri]::EscapeDataString("userPrincipalName eq '$userPrincipalName'")
                $userUri = "https://graph.microsoft.com/v1.0/users?`$filter=$filter&`$select=assignedLicenses"
                $userResult = Invoke-GraphGet -Uri $userUri
                ...
                Write-Warning "Could not retrieve licenses for $emailAddress : $($_.Exception.Message)"
            }
            
            if ($isSharedMailbox) {
                if ($userLicenses.Count -eq 0) {
                    $hasValidLicense = $true
                    Write-Output "Shared mailbox (unlicensed): $($_.DisplayName) ($($_.PrimarySmtpAddress))"
                }
                elseif ($userLicenses -contains 'EXCHANGEENTERPRISE') {
                    $hasValidLicense = $true
                    Write-Output "Shared mailbox (Exchange Online Plan 2): $($_.DisplayName) ($($_.PrimarySmtpAddress))"
                }
                elseif (($userLicenses -contains 'EXCHANGEARCHIVE_ADDON') -and ($userLicenses -contains 'EXCHANGESTANDARD')) {
                    $hasValidLicense = $true
                    Write-Output "Shared mailbox (Exchange Online Plan 1 + Archiving): $($_.DisplayName) ($($_.PrimarySmtpAddress))"
                }
                elseif ($userLicenses | Where-Object { $validSkus -contains $_ }) {
                    $hasValidLicense = $true
                    $matchedSku = ($userLicenses | Where-Object { $validSkus -contains $_ })[0]
                    Write-Output "Shared mailbox (licensed with $matchedSku): $($_.DisplayName) ($($_.PrimarySmtpAddress))"
                }
                else {
                    $hasValidLicense = $false
                    Write-Output "SKIPPED (Shared mailbox with invalid license): $($_.DisplayName) ($($_.PrimarySmtpAddress)) - Licenses: $($userLicenses -join ', ')"
                }
            }
            ...
            if (-not $hasValidLicense) {
                ...
                if (-not $isSharedMailbox) {
                    Write-Output "SKIPPED (No valid license): $($_.DisplayName) ($($_.PrimarySmtpAddress))"
                }
                return
            }
            ...
            while (-not $success -and $retryCount -lt $maxRetries) {
                try {
                    $mbxCheck = Get-Mailbox -Identity $_.PrimarySmtpAddress -ErrorAction Stop
                    
                    if ($mbxCheck.RetentionPolicy -eq $RetentionPolicyName) {
                        $policyAlreadyAssignedCount++
                        Write-Output "Retention policy already assigned: $($_.DisplayName) ($($_.PrimarySmtpAddress))"
                        $success = $true
                    }
                    else {
                        if ($WhatIf) {
                            Write-output "Whatif: Would set retention policy '$RetentionPolicyName' on $($_.DisplayName) ($($_.PrimarySmtpAddress))"
                            Set-Mailbox -Identity $_.PrimarySmtpAddress -RetentionPolicy $RetentionPolicyName -WhatIf -ErrorAction Stop
                            $assignedCount++
                            $success = $true
                        }
                        else {
                            Write-Output "Set retention policy '$RetentionPolicyName' on $($_.DisplayName) ($($_.PrimarySmtpAddress))"
                            Set-Mailbox -Identity $_.PrimarySmtpAddress -RetentionPolicy $RetentionPolicyName -ErrorAction Stop
                            $assignedCount++
                            $success = $true
                        }
                    }
                    ...
                    if ($errorMessage -match "server side error|try again after some time") {
                        if ($retryCount -lt $maxRetries) {
                            Write-Warning "Transient error for $($_.PrimarySmtpAddress), retry $retryCount of $maxRetries after 2 seconds..."
                            Start-Sleep -Seconds 2
                        }
                        else {
                            $transientErrorCount++
                            Write-Error "TRANSIENT ERROR (max retries): $($_.DisplayName) ($($_.PrimarySmtpAddress)) - $errorMessage"
                        }
                    }
                    else {
                        $errorCount++
                        Write-Error "FAILED: $($_.DisplayName) ($($_.PrimarySmtpAddress)) - $errorMessage"
                        break
                    }
                }
            }
            ...
            if ($success) {
                if ($isExemptFromArchive) {
                    $skippedCount++
                    Write-Output "  SKIPPED archive enablement (exempt group member): $($_.DisplayName) ($($_.PrimarySmtpAddress))"
                }
                else {
                    try {
                        $mbx = Get-Mailbox -Identity $_.PrimarySmtpAddress -ErrorAction Stop
                        
                        if ($mbx.ArchiveStatus -eq 'Active') {
                            $archiveAlreadyEnabledCount++
                            Write-Output "  Archive already enabled for $($_.DisplayName)"
                        }
                        else {
                            if ($WhatIf) {
                                Write-Output "  Whatif: Would enable archive for $($_.DisplayName)"
                                Enable-Mailbox -Identity $_.PrimarySmtpAddress -Archive -WhatIf -ErrorAction Stop
                                $archiveEnabledCount++
                            }
                            else {
                                Write-Output "  Enabling archive for $($_.DisplayName)"
                                Enable-Mailbox -Identity $_.PrimarySmtpAddress -Archive -ErrorAction Stop
                                $archiveEnabledCount++
                                Write-Output "  Archive enabled successfully for $($_.DisplayName)"
                            }
                        }
                    }
                    catch {
                        $archiveErrorCount++
                        Write-Error "  ARCHIVE FAILED: $($_.DisplayName) ($($_.PrimarySmtpAddress)) - $($_.Exception.Message)"
                    }
                }
            }
        }
            ...
            $errorCount++
            Write-Error "FAILED (outer): $($_.DisplayName) ($($_.PrimarySmtpAddress)) - Error: $($_.Exception.Message)"
        }
