...
$PSDefaultParameterValues = @{
    'Add-LabMachineDefinition:DomainName' = 'world.local'
    'Add-LabMachineDefinition:OperatingSystem' = 'Windows Server 2022 Standard (Desktop Experience)'
}

Add-LabVirtualNetworkDefinition -Name $labName -AddressSpace 192.168.1.0/24

Add-LabMachineDefinition -Name Lab2019DC1 -Roles RootDC -Memory 1GB -IpAddress 192.168.1.10 -

#remove the ISOpath reference below since that breaks the lab
$role = Get-LabPostInstallationActivity -CustomRole Exchange2019 -Properties @{ OrganizationName = 'World'} # ; IsoPath = "$labSources\ISOs\ExchangeServer2019-x64-CU9.iso" }

Add-LabMachineDefinition -Name Lab2019Ex1 -Memory 6GB -PostInstallationActivity $role -IpAddress 192.168.1.12 

Add-LabMachineDefinition -Name Lab2019Client -Memory 2GB -OperatingSystem 'Windows 11 Enterprise' -IpAddress 192.168.1.13
...
$role = Get-LabPostInstallationActivity -CustomRole Exchange2019 -Properties @{ OrganizationName = 'World'} #; IsoPath = "$labSources\ISOs\ExchangeServer2019-x64-cu15.iso" }
Add-LabMachineDefinition -Name E1Ex1 -Memory 6GB -PostInstallationActivity $role

Add-LabMachineDefinition -Name E1Client -Memory 2GB -OperatingSystem 'Windows 11 Enterprise'
