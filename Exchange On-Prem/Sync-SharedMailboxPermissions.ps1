<#
.SYNOPSIS
    Automatically manages shared mailbox permissions based on group membership with SM- prefix (Exchange On-Premises).

.DESCRIPTION
    This script synchronizes permissions between distribution groups and shared mailboxes.
    ...
    Uses Kerberos authentication to connect to Exchange On-Premises servers.

.PARAMETER GroupPrefix
    Prefix for groups that manage shared mailbox access. Default: "SMBX-"

.PARAMETER ExchangeDomain
    Domain name for Exchange servers. Default: "dt2kmeta.dansketraelast.dk"

.PARAMETER WhatIf
    If set to $true, shows what would be done without making any changes (dry-run mode)

.EXAMPLE
    .\Sync-SharedMailboxPermissions.ps1 -GroupPrefix "SMBX-"

.EXAMPLE
    .\Sync-SharedMailboxPermissions.ps1 -GroupPrefix "SMBX-" -WhatIf

.EXAMPLE
    .\Sync-SharedMailboxPermissions.ps1 -GroupPrefix "SMBX-" -ExchangeDomain "contoso.local"

.NOTES
    Requires: Exchange Management Shell or Remote PowerShell session to Exchange On-Premises
    Exchange Server: 2010 SP1+ (for AutoMapping support)
...
function Connect-ExchangeOnPremise {
    <#
    .SYNOPSIS
        Connects to Exchange On-Premises using Remote PowerShell with Kerberos authentication
    #>
    [CmdletBinding()]
    param(
    ...
    try {
        Write-Output "Connecting to Exchange On-Premises..."
        Write-Output "Domain: $Domain"
        
        foreach ($server in $servers) {
            if ($script:Session) { break }
            
            try {
                Write-Output "Attempting connection to $server..."
                $script:Session = New-PSSession -ConfigurationName Microsoft.Exchange `
                    -ConnectionUri "http://$server.$Domain/PowerShell/" `
                    -Authentication Kerberos `
                    -ErrorAction Stop
                
                Import-PSSession $script:Session -DisableNameChecking -AllowClobber -ErrorAction Stop | Out-Null
                Write-Output "Successfully connected to Exchange server: $server"
            }
            catch {
        ...
        if (-not $script:Session) {
            throw "Failed to connect to any Exchange server"
        }
    }
    catch {
        Write-Output "ERROR: Failed to connect to Exchange On-Premises: $_"
...
try {
    ...
    Write-Output "(Exchange On-Premises)"
    if ($WhatIf) {
    ...
    Write-Output "Exchange Domain: $ExchangeDomain"
    Write-Output "WhatIf Mode: $WhatIf"
    Write-Output ""
    
    Connect-ExchangeOnPremise -Domain $ExchangeDomain
    ...
    if ($script:Session) {
        Remove-PSSession $script:Session -Confirm:$false -ErrorAction SilentlyContinue
    }
